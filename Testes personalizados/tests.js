...
QUnit.module( "Gerenciar Participantes", function() {
  ...
  QUnit.test( "adicionar participantes", function( assert ) {
	...
	sistema.adicionarParticipante("Otavio", "Oliveira", "otaveira@gmail.com", 19, 2);
  ...
  QUnit.test( "adicionar participante em duplicidade", function( assert ) {
	...
	sistema.adicionarParticipante("Fernando", "Siqueira", "fernando@gmail.com", 32, 2);
    assert.throws(function(){
        sistema.adicionarParticipante("Fernando", "Siqueira", "fernando@gmail.com", 32, 2)
      }
    );
    sistema.removerParticipante("fernando@gmail");
  ...
  QUnit.test( "remover participante", function( assert ) {    
	...
	sistema.adicionarParticipante("Paulo Ricardo", "Sousa", "engcompaulo@gmail.com", 22, 2);
    sistema.removerParticipante("engcompaulo@gmail.com");
    var paulo = sistema.obterParticipante("engcompaulo@gmail.com");
    assert.ok(!paulo, "Não foi encontrado o registro");
...
QUnit.module( "Buscar Participantes", function() {
  ...
  QUnit.test( "buscar por aprovados e reprovados", function( assert ) {
	...
	sistema.adicionarNotaAoParticipante("cmendes@matrix.com", 50);
    var resultados = sistema.buscarParticipantesAprovados();    
    assert.equal( resultados.length, 1, "1 Participante reprovado");  
